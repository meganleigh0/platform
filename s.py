...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col(dcc.RadioItems(id='duration-selector', options=[{'label': 'Day', 'value': 1}, {'label': 'Week', 'value': 7}, {'label': 'Month', 'value': 30}], value=1), width=4)
    ]),
    dbc.Row([
...
def update_output(run_clicks, clear_clicks, selected_month, rate, duration):
    ...
    if ctx.triggered and ctx.triggered[0]['prop_id'] == 'run-simulation-btn.n_clicks':
        ...
        return gantt_chart, schedule_data.to_dict('records'), columns
